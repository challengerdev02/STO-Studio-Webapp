...
function pointAddScalar(p: any, tweak: any, isCompressed: boolean) {
...
function pointMultiply(p: any, tweak: any, isCompressed: boolean) {
  const P = necc.Point.fromHex(p);
...
function privateAdd(privateKey: any, tweak: any) {
...
function privateNegate(privateKey: Uint8Array): Uint8Array | null {
  const p = normalizePrivateKey(privateKey);
...
const ecc = {
  ...
  privateNegate: (d: Uint8Array): Uint8Array => privateNegate(d) as Uint8Array,
