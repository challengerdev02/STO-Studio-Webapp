...
describe('BrowseNFT', function () {
  ...
  let props = {
    browseAuctionNfts: shuffle(images).map(
      (nft) =>
        ({
          ...market_card,
  ...
  const setup = (extraProps: Record<string, any> = {}) => {
    const utils = render(<BrowseNFT {...props} {...extraProps} />);
    return {
      ...utils,
    };
  };
  ...
  it('should display the BrowseNFT', () => {
    // @ts-ignore
    const { container } = setup();
    expect(queryAllByRole(document.body, 'halfs').length).toBe(2);
  });
  it('should Toggle', async () => {
    ...
    await waitFor(() => {
      const btn = queryByRole(document.body, 'filter-toggle-btn');
    ...
    await waitFor(() => {
      expect(
        queryByAttribute('class', document.body, 'mobile-mfc hide')
      ).toBeInTheDocument();
    });
  });

  it('should match with snapshot "showLoad: false"', () => {
    // @ts-ignore
    const { container } = setup({ showLoad: false });
    expect(container).toMatchSnapshot();
  });

  it('should match with snapshot "loadingState: false"', () => {
    // @ts-ignore
    const { container } = setup({ loadingState: false });
    expect(container).toMatchSnapshot();
  });
});
