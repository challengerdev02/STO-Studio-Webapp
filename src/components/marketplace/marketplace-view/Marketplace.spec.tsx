...
describe('MarketPlace', function () {
  ...
  let props = {
    featuredNfts: shuffle(images).map((nft) => ({
      imageUrl: nft.src,
      name: '',
    })),
    liveAuctionNfts: shuffle(images).map((nft) => ({
      ...market_card,
      coverInfo: { ...market_card.coverInfo, coverImg: nft.src },
    })),
    lunchingAuctionNfts: shuffle(images).map((nft) => ({
      ...market_card,
      coverInfo: { ...market_card.coverInfo, coverImg: nft.src },
    })),
    trendingNfts: shuffle(images)
      .splice(0, 6)
      .map((nft) => ({ imageUrl: nft.src, name: '' })),
    browse: {
  ...
  const setup = (extraProps: Record<string, any> = {}) => {
    const utils = render(<MarketPlace {...props} {...extraProps} />);
    return {
      ...utils,
    };
  };
  ...
  it('should display the MarketPlace', () => {
    // @ts-ignore
    const { container } = setup();
    expect(queryByText(document.body, 'Featured')).toBeInTheDocument();
    expect(queryByText(document.body, 'Trending')).toBeInTheDocument();
    expect(queryByText(document.body, 'Live Auction 🔥')).toBeInTheDocument();
    expect(queryByText(document.body, 'Launching Soon 🔥')).toBeInTheDocument();
    expect(queryByText(document.body, 'Browse')).toBeInTheDocument();
  });
});
