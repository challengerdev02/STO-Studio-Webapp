...
import { CreateSceneContainer } from './index';
...
describe('Create Asset Container', function () {
  ...
  it('should match with snapshot', () => {
    // @ts-ignore
    const { container } = render(<CreateSceneContainer />);
    expect(container).toMatchSnapshot();
  });

  it('should match with snapshot', () => {
    // @ts-ignore
    const { container } = render(<CreateSceneContainer assetDomain="revise" />);
    expect(container).toMatchSnapshot();
  });
  ...
  it('should try to submit New Scene', async () => {
    // @ts-ignore
    const { container } = render(<CreateSceneContainer />);
    ...
    await waitFor(() => {
      const inputName = queryAllByPlaceholderText(
        document.body,
        `e. g. Redeemable One Comic Sence One`
      );
  ...
  it('should try to submit Revise', async () => {
    // @ts-ignore
    const { container } = render(<CreateSceneContainer assetDomain="revise" />);

    await waitFor(() => {
      const input = queryAllByPlaceholderText(document.body, 'Artist URL');

      if (input) {
        fireEvent.change(input[0], { target: { value: 'mike' } });
        fireEvent.change(input[1], { target: { value: 'mike' } });
      }
    });

    await waitFor(() => {
      const btn = queryByText(document.body, 'Revise Item');
      if (btn) {
        fireEvent.click(btn);
      }
    });
  ...
  it('should fire upload Cover Image', async () => {
    ...
    await waitFor(() => {
      expect(mockHandleUpdateScene).toHaveBeenCalled();
      expect(mockBack).toHaveBeenCalled();
    });
  });

  it('should submit without cover image ', async () => {
    // @ts-ignore
